...
function handleKeyDown(event: KeyboardEvent) {
  ...
  const maxIndex = props.categories.length;
  ...
  if (event.key === "Escape") {
    ...
    event.preventDefault();
    focusedIndex.value = (focusedIndex.value + 1) % maxIndex;
    scrollToFocused();
  } else if (event.key === "ArrowUp") {
    event.preventDefault();
    focusedIndex.value = (focusedIndex.value - 1 + maxIndex) % maxIndex;
    scrollToFocused();
  } else if (event.key === "Enter" || event.key === " ") {
    event.preventDefault();
    if (focusedIndex.value >= 0) {
      selectCategory(props.categories[focusedIndex.value]);
    }
...
<template>
  <div class="cs-select" ref="containerRef" @keydown="handleKeyDown">
    <div class="cs-select-trigger" :class="{ 'is-open': isOpen }" @click="toggleDropdown" tabindex="0">
      <div class="cs-select-value" :class="{ 'is-placeholder': !modelValue }">
        {{ displayValue }}
      </div>
      ...
      <div v-if="isOpen" class="cs-select-dropdown" ref="listboxRef" tabindex="-1" :style="dropdownStyle">
        <div class="cs-select-list">
          <div
            ...
            :class="{ 'is-focused': focusedIndex === index, 'is-selected': modelValue === cat }"
